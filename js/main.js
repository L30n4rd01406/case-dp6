(function (jQuery) {
  ...
  jQuery('.contato').on('submit', function (e) {
    ...
    var formId = $(".contato").attr("id") || "";
    var formName = $(".contato").attr("name") || "";
    e.preventDefault();
    setTimeout(function () {
      console.log('enviado');
      // evento de sucesso do formulario
      viewFormSuccessGA4(formId, formName);
      lb.find('.lightbox-icon')
...
// evento de click menu
 $(".menu-lista-link:not(.menu-lista-download)").on("click", function() {
    eventClickGA4($(this).text(), "menu")
 });

 // evento de click sub_menu
 $(".menu-sublista-link").on("click", function() {
    eventClickGA4($(this).text(), "sub_menu")
 });

// evento de click download PDF
 $(".menu-lista-download").on("click", function() {
    eventFileDownloadGA4($(this).text(), "menu")
 });
